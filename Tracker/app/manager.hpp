...
{
private:
	std::unordered_map<std::string, Sampler::Shared> samplers;
...
public:
	...
	void set( std::string key , Sampler::Shared& sampler );

	bool get( std::string key , Material& material ) const;
	bool get( std::string key , Sampler::Shared& sampler ) const;
